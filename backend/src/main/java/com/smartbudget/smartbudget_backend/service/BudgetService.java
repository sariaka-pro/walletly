...
import com.smartbudget.smartbudget_backend.dto.YearlyBudgetStatsDto;
...
public class BudgetService {
   ...
   public BigDecimal getPercentageSpent(Budget budget) {
     ...
     */
    public YearlyBudgetStatsDto getYearlyStats(Long userId, int year) {
        List<Budget> budgets = budgetRepository.findByUserIdAndYear(userId, year);
    
        // Sommes avec BigDecimal
        BigDecimal totalSpendingLimit = budgets.stream()
            .map(Budget::getSpendingLimit)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
    
        BigDecimal totalSpent = budgets.stream()
            .map(Budget::getCurrentSpent)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
    
        // Calcul du restant
        BigDecimal totalRemaining = totalSpendingLimit.subtract(totalSpent);
    
        // Calcul du pourcentage dépensé
        BigDecimal percentageSpent = BigDecimal.ZERO;
        if (totalSpendingLimit.compareTo(BigDecimal.ZERO) > 0) {
            percentageSpent = totalSpent
                .divide(totalSpendingLimit, 2, RoundingMode.HALF_UP) // 2 décimales
                .multiply(BigDecimal.valueOf(100));
        }
    
        // Construction du DTO
        return YearlyBudgetStatsDto.builder()
            .totalLimit(totalSpendingLimit)
            .totalSpent(totalSpent)
            .totalRemaining(totalRemaining)
            .percentageSpent(percentageSpent)
            .monthlyBreakdown(budgets)
            .build();
    }}